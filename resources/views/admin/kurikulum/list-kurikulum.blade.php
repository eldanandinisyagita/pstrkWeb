               ...
               <table class="table table-hoverr">
                    ...
                    <tbody id="table-kurikulums">
                        ...
                        <tr id="index_{{ $kurikulum->id }}">
                            <td class="row-index">{{ ($kurikulums->currentPage() - 1) * $kurikulums->perPage() + $loop->iteration }}</td>
                            <td>{{ $kurikulum->kode_mk }}</td>
                            <td>{{ $kurikulum->nama_mk }}</td>
                            <td>{{ $kurikulum->smstr }}</td>
                            <td class="text-center" style="padding-right:10px">
                                <a href="javascript:void(0)" id="btn-detail" data-id="{{ $kurikulum->id }}" class="button-detail btn btn-sm">detail</a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                ...
                @include('admin.kurikulum.modal-detail')
                ...
                <nav aria-label="Page navigation">
                    <ul class="pagination">
                        ...
                        <li class="page-item {{ !$kurikulums->hasMorePages() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $kurikulums->nextPageUrl() }}">Selanjutnya</a>
                        </li>
                    </ul>
                </nav>
            </div>
