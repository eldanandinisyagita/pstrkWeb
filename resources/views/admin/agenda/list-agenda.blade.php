                ...
                <div class="table-responsive">
                   ...
                   <table class="table table-hoverr">
                        ...
                        <tr id="index_{{ $agenda->id }}">
                                <td class="row-index">{{ ($agendas->currentPage() - 1) * $agendas->perPage() + $loop->iteration }}</td>
                                <td>{{$agenda->judul}}</td>
                                <td>{{$agenda->lokasi}}</td>
                                <td>{{$agenda->tgl_mulai}}</td>
                                <td class="text-center" style="padding-right:10px">
                                    <a href="javascript:void(0)" id="btn-detail" data-id="{{ $agenda->id }}" class="button-detail btn btn-sm">detail</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                    ...
                    @include('admin.agenda.modal-detail')
                    ...
                    <nav aria-label="Page navigation">
                        <ul class="pagination">
                            ...
                            <li class="page-item {{ $agendas->onFirstPage() ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $agendas->previousPageUrl() }}" style="te">Sebelumnya</a>
                            </li>
                            ...
                            <li class="page-item {{ !$agendas->hasMorePages() ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $agendas->nextPageUrl() }}">Selanjutnya</a>
                            </li>
                        </ul>
                    </nav>
                </div>
